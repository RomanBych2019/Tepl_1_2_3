...
public:
    ...
    Teplica(int id, Sensor_WB_v_3 *Sensor_WB, int relayPump, int relayUp, int relayDown,
            int setpump, int setheat, int setwindow, int opentimewindows, MB11016P_ESP *mb11016p, Preferences *flash) : _mode(AUTO), _id(id), _setpump(setpump), _setheat(setheat), _setwindow(setwindow), _relayPump(relayPump),
                                                                                                                        _relayUp(relayUp), _relayDown(relayDown)
    {
    ...
    void regulationWindow(int temperature, int outdoortemperature = 0)
    {
        ...
        if (getMode() == AUTO)
        {
            ...
            else if (_oldtemperatute <= temperature) // если температура не падает
            {
                ...
                {
                    __window->openWindow(20, _id);
                    _temperatureIntegral = 0;
                }
                else if (temperature > _setwindow + 50)
                    if (_temperatureIntegral > 100) //
                    {
                        __window->openWindow(constrain(map(_counter, 1, 16, 12, 5), 5, 15), _id);
                        _temperatureIntegral = 0;
                        _counter = 0;
                    }
            }
            ...
            else
            {
                ...
                {
                    __window->closeWindow(constrain(map(_counter, 1, 16, 15, 5), 5, 15), _id);
                    _temperatureIntegral = 0;
                    _counter = 0;
                }
    ...
    void air(unsigned long airtime, int outdoortemperature)
    {
        ...
        if (__relay->getRelay(_relayUp) || __relay->getRelay(_relayDown))
            return;
    ...
    // установка уровня окон
    void setWindowlevel(uint level) const
    {
        __window->setLevel(level);
    }
